...
bool CheckSpendZoinTransaction(const CTransaction &tx,
	...
	BOOST_FOREACH(const CTxIn &txin, tx.vin)
	{
        ...
        if (txin.scriptSig.size() < 4)
            return state.DoS(100,
                             false,
                             REJECT_MALFORMED,
                             "CheckSpendZoinTransaction: invalid spend transaction");
        ...
        if (!zerocoinState.GetCoinGroupInfo(targetDenomination, pubcoinId, coinGroup))
            return state.DoS(100, false, NO_MINT_ZEROCOIN, "CheckSpendZoinTransaction: Error: no coins were minted with such parameters");
        ...
        do {
            if ((index->*accChanges).count(denominationAndId) > 0) {
                libzerocoin::Accumulator accumulator(zcParams,
                                                     ...
                                                     targetDenomination);
                LogPrintf("CheckSpendZoinTransaction: accumulator=%s\n", accumulator.getValue().ToString().substr(0,15));
        ...
        if (!passVerify && spendVersion == ZEROCOIN_TX_VERSION_1) {
            ...
            if (!passVerify) {
                ...
                BOOST_REVERSE_FOREACH(const CBigNum &pubCoin, pubCoins) {
                    accumulator += libzerocoin::PublicCoin(zcParams, pubCoin, (libzerocoin::CoinDenomination)targetDenomination);
                    LogPrintf("CheckSpendZoinTransaction: accumulatorRev=%s\n", accumulator.getValue().ToString().substr(0,15));
                    if ((passVerify = newSpend.Verify(accumulator, newMetadata)) == true)
                        break;
                }
            }
        ...
        if (passVerify) {
            ...
            LogPrintf("CheckSpendZoinTransaction: verification failed at block %d\n", nHeight);
            return false;
        }
	}
	return true;
...
bool CheckMintZoinTransaction(const CTxOut &txout,
                               CValidationState &state,
                               uint256 hashTx,
                               CZerocoinTxInfo *zerocoinTxInfo) {

    LogPrintf("CheckMintZoinTransaction txHash = %s\n", txout.GetHash().ToString());
    LogPrintf("nValue = %d\n", txout.nValue);
                              ...
                              CZerocoinTxInfo *zerocoinTxInfo)
{
	...
	BOOST_FOREACH(const CTxOut &txout, tx.vout) {
		if (!txout.scriptPubKey.empty() && txout.scriptPubKey.IsZerocoinMint()) {
            if (!CheckMintZoinTransaction(txout, state, hashTx, zerocoinTxInfo))
                return false;
		}
	}
	...
	if(tx.IsZerocoinSpend()) {
		...
		BOOST_FOREACH(const CTxOut &txout, tx.vout)
		{
			if (!isVerifyDB) {
                ...
                case libzerocoin::ZQ_PEDERSEN*COIN:
                case libzerocoin::ZQ_WILLIAMSON*COIN:
                    if(!CheckSpendZoinTransaction(tx, (libzerocoin::CoinDenomination)(txout.nValue / COIN), state, hashTx, isVerifyDB, nHeight, isCheckWallet, zerocoinTxInfo))
                        return false;
                    break;
